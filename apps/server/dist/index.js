...
function startMatchTimer(match) {
    const updateTimer = () => {
        ...
        if (match.remainingTime <= 0) {
            ...
            let winner = null;
            let loser = null;
            const player1Health = (_a = match.player1.health) !== null && _a !== void 0 ? _a : 100;
            const player2Health = (_b = match.player2.health) !== null && _b !== void 0 ? _b : 100;
            ...
            if (matchId !== null) {
                states_1.PLAYER_MATCH.delete(match.player1.id);
                states_1.PLAYER_MATCH.delete(match.player2.id);
                delete states_1.MATCHES[matchId];
            }
            if (match.timer) {
...
function createMatch(player1Id, player2Id) {
    ...
    // Store the match
    states_1.MATCHES[matchId] = match;
    // Add players to the match room
    player1Socket.join(roomId);
    player2Socket.join(roomId);
    // Set up quick lookup from player ID to match ID
    states_1.PLAYER_MATCH.set(player1Id, matchId);
    states_1.PLAYER_MATCH.set(player2Id, matchId);
    // Start match timer
...
io.on("connection", (socket) => {
    ...
    socket.on("playerMoved", (data) => {
        const matchId = states_1.PLAYER_MATCH.get(socket.id);
        if (matchId === undefined) {
            return;
        }
        const match = states_1.MATCHES[matchId];
        if (!match) {
            return;
        }
            ...
            if (match.player2.x !== undefined &&
                ...
                if (match.player2.health <= 0) {
                    ...
                    if (cleanupMatchId !== null) {
                        states_1.PLAYER_MATCH.delete(match.player1.id);
                        states_1.PLAYER_MATCH.delete(match.player2.id);
                        delete states_1.MATCHES[cleanupMatchId];
                    }
                    return;
                }
            }
