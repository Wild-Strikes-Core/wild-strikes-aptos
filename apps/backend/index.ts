...
io.on('connection', socket => {
  ...
  socket.on('playerInput', data => {
    ...
    // Basic integration – move position based on dirX and run flag; apply jump once
    const speed = data.run ? 400 : 200;
    const delta = UPDATE_THROTTLE / 1000; // seconds per tick

    player.x = (player.x ?? 0) + data.dirX * speed * delta;
    ...
    player.flipX = data.dirX < 0;
    player.animation = data.attack ? '_Attack2' : data.dirX !== 0 ? '_Run' : '_Idle_Idle';
