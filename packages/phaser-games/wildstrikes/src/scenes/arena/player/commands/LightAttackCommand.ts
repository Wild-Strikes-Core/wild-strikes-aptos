...
export class LightAttackCommand implements ScalableCommand {
    ...
    executeRemote(player: PlayerManager): void {
        ...
        spriteManager.setLightAttackCompleteCallback(() => {
            // Transition to idle after animation completes (for remote players)
            console.log('[REMOTE] Light attack animation completed, transitioning to idle');
            player.transitionTo(PlayerStates.Idle);
        });
        
        spriteManager.playAttackingAnimation(playerSprite);
    }
