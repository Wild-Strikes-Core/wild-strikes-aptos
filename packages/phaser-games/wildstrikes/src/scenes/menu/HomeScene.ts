...
export class HomeScene extends BaseScene {
    ...
    //Character 
    private BG_KNIGHT!: Phaser.GameObjects.Image;

    // Ranks and Shards
    private RANK_ICON!: Phaser.GameObjects.Image;
    ...
    create(): void {
        ...
        this.time.delayedCall(800, this.setupButtonInteractions, [], this);
    }
    ...
    private createBackground(): void {
        ...
        if (this.textures.exists('2G_bgHill')) {
            this.BG_HILL = this.add.image(this.cameras.main.centerX, height, '2G_bgHill').setOrigin(0.5, 1).setDepth(-800).setScale(1.8); // Increased hill scale
        }

        if (this.textures.exists('2G_Char_Knight')) {
            const knightX = 550;      
            const knightY = height - 190; 
            const knightScale = 1;         
            const knightDepth = 500;       

            this.BG_KNIGHT = this.add.image(knightX, knightY, '2G_Char_Knight')
                .setOrigin(0.5, 1)    
                .setScale(knightScale)
                .setDepth(knightDepth)
        }
    }
    ...
    private createMenuButtons(): void {
        this.BUTTON_ARENA = this.add.image(1500,700, '2G_btnArena').setScale(1.1);
        this.MENU_BUTTONS.add(this.BUTTON_ARENA);
    }
    
    private createRankAndShards(): void {
        const arenaX = this.BUTTON_ARENA.x;
        const arenaY = this.BUTTON_ARENA.y;

        this.RANK_ICON = this.add.image(arenaX, arenaY - 400, 'rank_icon').setScale(2.1);
        this.MENU_BUTTONS.add(this.RANK_ICON);

        const shardCount = 3;
        const spacing = 120; 
        const shardsY = arenaY - 175; 
        const startX = arenaX - (spacing * (shardCount - 1)) / 2; 

        for (let i = 0; i < shardCount; i++) {
            const shard = this.add.image(startX + i * spacing, shardsY, 'Full-Shards').setScale(0.9);
            this.SHARDS.push(shard);
            this.MENU_BUTTONS.add(shard);
        }
    }
    // --- END NEW ---
    ...
    private createBottomMenuButtons(): void {
        ...
        const dailyLeaderboardSpacing = 250;

        const leftStartX = width * 0.1;
        const buttonScale = 1.3; 
        this.DAILY_CHEST = this.add.image(leftStartX, buttonY, '2G_btnDaily').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.DAILY_CHEST);

        this.BUTTON_LEADERBOARDS = this.add.image(leftStartX + dailyLeaderboardSpacing, buttonY, '2G_btnLeaderboards').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.BUTTON_LEADERBOARDS);

        this.SHOP = this.add.image(leftStartX + dailyLeaderboardSpacing + spacing, buttonY, '2G_btnShop').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.SHOP);

        this.EVENTS = this.add.image(leftStartX + dailyLeaderboardSpacing + (2 * spacing), buttonY, '2G_btnEvents').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.EVENTS);

        const rightStartX = width * 0.9; 
        this.INVENTORY = this.add.image(rightStartX - spacing, buttonY, '2G_btnInventory').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.INVENTORY);

        this.SETTINGS = this.add.image(rightStartX, buttonY, '2G_btnSettings').setScale(buttonScale);
        this.BOTTOM_BUTTONS.add(this.SETTINGS);
    }

    private createPlayerCard(): void {
        const playerCardScale = 1.2;
        if (this.textures.exists('2g_icoAvatar')) {
            this.PLAYER_AVATAR = this.add.image(200, 158, '2g_icoAvatar')
                .setScale(0.7 * playerCardScale)
                .setAlpha(0);
        }

        this.PLAYER_NAME = this.add.text(394, 138, 'USERNAME', {
        ...
        this.PLAYER_USERID = this.add.text(444, 188, 'User ID: 123456789', { 
            fontFamily: '"VT323", monospace',
            fontSize: '40px', 
            align: 'center',
            color: '#ffffffff',
    ...
    private setInitialStates(): void {
        // Menu buttons
        const menuButtons = [...this.getMenuButtons(), this.RANK_ICON, ...this.SHARDS];
    ...
    private animatePlayerCard(): void {
        ...
        const targets: Phaser.GameObjects.GameObject[] = [this.PLAYER_NAME, this.PLAYER_USERID];
        if (this.PLAYER_AVATAR) {
    ...
    private animateMenuButtons(): void {
        const buttons = [...this.getMenuButtons(), this.RANK_ICON, ...this.SHARDS];
            ...
            buttons.forEach((btn, index) => {
                ...
                const tweenConfig: Phaser.Types.Tweens.TweenBuilderConfig = {
                    ...
                    onComplete: () => {
                        ...
                        if (this.SHARDS.includes(btn as Phaser.GameObjects.Image)) {
                            distance = 4;
                            duration = 1400 + Math.random() * 400; 
                        } else if (btn === this.RANK_ICON) {
                            duration = 1300; 
                        }
    ...
    private setupButtonInteractions(): void {
        ...
        this.setupSecondaryButton(this.SETTINGS, 'SettingsScene'); 
    }

    private setupBaseButton(button: Phaser.GameObjects.Image): void {
        // @ts-ignore
        button.finalScaleX = button.scaleX;
        // @ts-ignore
        button.finalScaleY = button.scaleY;
        ...
        button
            ...
            .on('pointerover', () => {
                ...
                this.tweens.add({
                    targets: button,
                    // @ts-ignore
                    scaleX: button.finalScaleX * 1.1,
                    // @ts-ignore
                    scaleY: button.finalScaleY * 1.1,
                    duration: 100
                });
            })
            .on('pointerout', () => {
                ...
                this.tweens.add({
                    targets: button,
                    // @ts-ignore
                    scaleX: button.finalScaleX,
                    // @ts-ignore
                    scaleY: button.finalScaleY,
                    duration: 100
                });
            });
    }
    ...
    private getBottomButtons(): Phaser.GameObjects.Image[] {
        ...
        if (this.SETTINGS) buttons.push(this.SETTINGS); 
        return buttons;
    }
    ...
    private getAllUIElements(): Phaser.GameObjects.GameObject[] {
        ...
        if (this.BUTTON_LEADERBOARDS) elements.push(this.BUTTON_LEADERBOARDS);
        ...
        if (this.SETTINGS) elements.push(this.SETTINGS);
        if (this.PLAYER_NAME) elements.push(this.PLAYER_NAME);
        if (this.PLAYER_AVATAR) elements.push(this.PLAYER_AVATAR);
        if (this.PLAYER_USERID) elements.push(this.PLAYER_USERID);
        if (this.RANK_ICON) elements.push(this.RANK_ICON);
        elements.push(...this.SHARDS);
